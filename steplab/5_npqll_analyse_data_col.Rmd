...
source("C:/Users/stefanie.meliss/OneDrive - Ambition Institute/code/AO_NPQLL/pilots/functions.R")
...
```{r, echo = F, results='asis', fig.align='center', warning=FALSE, out.width='100%', out.height='100%'}
...
plot(m1,
     family = "Segoe UI",
     cook.legendChanges = list(x = "topright"))

```

```{r, include=F}
# save plot to file
png(filename = "plot.lm1.png", width = 1080, height = 1240, res = 300)
plot(m1,
     which = 1, #1, 2, 3, 5
     #caption = "",
     family = "Segoe UI",
...
png(filename = "plot.lm2.png", width = 1080, height = 1240, res = 300)
plot(m1,
     which = 2, #1, 2, 3, 5
     #caption = "",
     family = "Segoe UI",
...
png(filename = "plot.lm3.png", width = 1080, height = 1240, res = 300)
plot(m1,
     which = 3, #1, 2, 3, 5
     #caption = "",
     family = "Segoe UI",
...
png(filename = "plot.lm4.png", width = 1080, height = 1240, res = 300)
plot(m1,
     which = 5, #1, 2, 3, 5
     #caption = "",
     family = "Segoe UI",
...
plt_all <- ggplot(data = df, aes(x = score_pretest_c, y = score_posttest_c)) +
  geom_point(color = nondominant_col, position = position_jitter(width = 0.1, height = 0.1)) +
  #facet_wrap(. ~ group, nrow = 2) + 
  ...
  ggpubr::stat_cor(method = "pearson", cor.coef.name = "r",
                   ...
                   label.y = 2.5, 
                   size = pt_to_mm(font_size), family = "Segoe UI")  # stat_regline_equation(label.y = 1.5, aes(label = ..eq.label..)) +
...
plt_grp <- ggplot(data = df, aes(x = score_pretest_c, y = score_posttest_c)) +
  geom_point(color = nondominant_col, position = position_jitter(width = 0.1, height = 0.1)) +
  facet_wrap(. ~ group, nrow = 2) + 
  stat_smooth(aes(col = group), formula = y ~ x, method = "lm", fullrange = T, se = T, alpha=0.2) +
  scale_color_manual(values = c(red,blue)) +
  ambition_theme + 
  theme(legend.position = "none") +
  #'scale_x_continuous(breaks=seq(0, 10, 2)) +
  xlab("Prior knowledge assessment") + ylab("") +
  ggpubr::stat_cor(method = "pearson", cor.coef.name = "r",
...
plt <- ggpubr::ggarrange(plt_all, plt_grp, 
                         ncol = 2,
                         labels = c("A", "B"),
                         font.label = list(size = head_size, family = "Segoe UI"))
plt
...
```{r, echo = F, results='asis', fig.align='center', warning=FALSE}
# jitter the data so that the position is the same throughout all three graphs
set.seed(123)
df$score_posttest_c_jitter <- jitter(df$score_posttest_c)
df$score_pretest_c_jitter <- jitter(df$score_pretest_c)


# scatterplot with all data
plt_all <- ggplot(data = df, aes(x = score_pretest_c_jitter, y = score_posttest_c_jitter)) +
  geom_point(color = nondominant_col) +
  stat_smooth(formula = y ~ x, method = "lm", fullrange = T, se = T, alpha=0.2, color = dominant_col) +
  ambition_theme + 
  coord_cartesian(xlim = c(-2.5,2.5), ylim = c(-2.5,2.5)) +
  xlab("Prior knowledge assessment") + ylab("Learning outcome assessment")
# export without cor coefs
ggsave("scatter_all_clean.jpeg",
       plot = plt_all,
       height = 1080,
       width = 3630,
       units = "px",
       dpi = 300)
# export with cor coefs
plt_all <- plt_all +
  ggpubr::stat_cor(method = "pearson", cor.coef.name = "r",
                   p.accuracy = 0.001, r.accuracy = 0.01,
                   label.y = 2.5, 
...
# scatterplot with AO data
ao <- subset(df, group == "AO")

plt_ao <- ggplot(data = ao, aes(x = score_pretest_c_jitter, y = score_posttest_c_jitter)) +
  geom_point(color = nondominant_col) +
  stat_smooth(formula = y ~ x, method = "lm", fullrange = T, se = T, alpha=0.2, color = red) +
  ambition_theme + 
  coord_cartesian(xlim = c(-2.5,2.5), ylim = c(-2.5,2.5)) +
  xlab("Prior knowledge assessment") + ylab("Learning outcome assessment")  
# export without cor coefs
ggsave("scatter_AO_clean.jpeg",
       plot = plt_ao,
       height = 1080,
       width = 3630,
       units = "px",
...
plt_ao <- plt_ao +
  ggpubr::stat_cor(method = "pearson", cor.coef.name = "r",
                   ...
                   size = pt_to_mm(font_size), family = "Segoe UI")  
plt_ao
ggsave("scatter_AO.jpeg",
       height = 1080,
       width = 3630,
       units = "px",
       dpi = 300)

# scatterplot with non-AO data
nao <- subset(df, group == "non-AO")

plt_nao <- ggplot(data = nao, aes(x = score_pretest_c_jitter, y = score_posttest_c_jitter)) +
  geom_point(color = nondominant_col) +
  stat_smooth(formula = y ~ x, method = "lm", fullrange = T, se = T, alpha=0.2, color = blue) +
  ambition_theme + 
  coord_cartesian(xlim = c(-2.5,2.5), ylim = c(-2.5,2.5)) +
  xlab("Prior knowledge assessment") + ylab("Learning outcome assessment")  
# export without cor coefs
ggsave("scatter_non-AO_clean.jpeg",
       plot = plt_nao,
       height = 1080,
       width = 3630,
       units = "px",
  ...
  coord_cartesian(xlim = c(-2.5,2.5), ylim = c(-2.5,2.5)) +
    theme(axis.title = element_blank(),
        ...
        plot.margin = margin(t = -1,  # Top margin
                             ...
                             l = -1))
# export without cor coefs
ggsave("scatter_all_vclean.jpeg",
       plot = plt_all,
       height = 1080,
       width = 3630,
       units = "px",
...
plt_ao <- ggplot(data = ao, aes(x = score_pretest_c, y = score_posttest_c)) +
  geom_point(color = nondominant_col, position = position_jitter(width = 0.1, height = 0.1)) +
  stat_smooth(formula = y ~ x, method = "lm", fullrange = T, se = T, alpha=0.2, color = red) +
  ambition_theme + 
  coord_cartesian(xlim = c(-2.5,2.5), ylim = c(-2.5,2.5)) +
...
plt_nao <- ggplot(data = nao, aes(x = score_pretest_c, y = score_posttest_c)) +
  geom_point(color = nondominant_col, position = position_jitter(width = 0.1, height = 0.1)) +
  stat_smooth(formula = y ~ x, method = "lm", fullrange = T, se = T, alpha=0.2, color = blue) +
  ambition_theme + 
  coord_cartesian(xlim = c(-2.5,2.5), ylim = c(-2.5,2.5)) +
  theme(axis.title = element_blank(),
        axis.text = element_blank())
# export without cor coefs
ggsave("scatter_non-AO_vclean.jpeg",
       plot = plt_nao,
       height = 1080,
       width = 3630,
       units = "px",
...
plt <- ggplot(data = emm, aes(x = group, y = emm)) +
  geom_errorbar(aes(ymin = emm - 1.96*se, ymax = emm + 1.96*se), width = .05, col = black) +
  geom_point(aes(col = group), size = 2) +
  scale_color_manual(values = c(red,blue)) +
  coord_cartesian(ylim = c(min(df$score_posttest), max(df$score_posttest))) +
  xlab("Introductory material") + ylab("Learning outcome assessment") +
...
write.csv(emm, "table_3.csv", row.names = F)
...
##### Prior knowledge median split

The theory surrounding AOs suggests that they are particularly beneficial if the learners are inexperienced. We hence repeated the ANCOVA analysis in a subset of participants that score below the sample median in the prior knowledge assessment.

```{r, echo = F, results='asis', fig.align='center', warning=FALSE}

cat("The sample median was ", median(df$score_pretest), ". \n", sep = "")


# check for outliers
df$below_median_pretest <- df$score_pretest < median(df$score_pretest)

if (sum(df$below_median_pretest) != 0 ) {
  cat("\nN =", sum(df$below_median_pretest), "participants were identified whose performance in the prior knowledge assessment was below the sample median. outliers were identified using the same IQR criteria described above. \n")
  median <- df[df$below_median_pretest, ]
}

# run ANOCOVA
m7 <- lm(score_posttest_c ~ group_d + score_pretest_c, data = median)
m <- lm(score_posttest ~ group + score_pretest, data = median)

# generate Ancova output
tmp <- car::Anova(m7, type = "III")
...
# add mean and se of covariate
emm$m_cov <- tapply(median$score_pretest, median$group, mean)
emm$se_cov <- tapply(median$score_pretest, median$group, se)

# add mean and se of DV
emm$m_dv <- tapply(median$score_posttest, median$group, mean)
emm$se_dv <- tapply(median$score_posttest, median$group, se)
...
cat('\n\n<!-- -->\n\n')
sjPlot::tab_model(m7, show.stat = T, show.fstat = T, show.df = T)


cat("\n")

kableExtra::kable(emm)
...
write.csv(out, "table_2.csv", row.names = T)
