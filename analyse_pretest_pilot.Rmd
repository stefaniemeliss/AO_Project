...
df <- read.csv("data_export_jan20_txt.csv", stringsAsFactors = F)
...
Participants were recruited using Twitter. The pilot study was set up in Qualtrics and a distribution link was shared (by SS). In total, the survey was completed by **101** participants. The project protocol was approved by University College London's ethics committee. Participants were informed about the purpose and provided informed consent to participate in the pilot study.   
...
ggplot(df, aes(x = Finished, y = q_total)) +
  ...
  geom_hline(yintercept = nrow(quest)/4, linetype="dashed") +
  ...
  coord_cartesian(ylim = c(0,nrow(quest)), xlim = c(1,1)) +
  theme_bw() +
  ggtitle("Descriptive visualisation of performance in prior knowledge assessment") +
...
After determining whether the total score was normally distributed, the appropriate statistical test (i.e., one-Sample t-test or the non-parametric Wilcoxon signed rank test) was used to determine whether the performance was significantly better than chance.
...
# create rain cloud plot, adapted from: https://z3tt.github.io/Rainclouds/
ggplot(df, aes(x = Finished, y = quest_amount_score)) +
  ggdist::stat_halfeye(adjust = 1.5, width = .3, .width = 0, justification = -.3, point_colour = NA, fill = ambition_charcole) + 
  geom_boxplot(width = .1, outlier.shape = NA) + # do not show outlier in boxplot
  stat_summary(fun="mean", geom = "point", col = ambition_coral) + 
  stat_summary(fun.data = mean_se, geom = "errorbar", width = .05, col = ambition_coral) +
...
# create rain cloud plot, adapted from: https://z3tt.github.io/Rainclouds/
ggplot(df, aes(x = Finished, y = quest_difficulty_score)) +
  ggdist::stat_halfeye(adjust = 1.5, width = .3, .width = 0, justification = -.3, point_colour = NA, fill = ambition_charcole) + 
  geom_boxplot(width = .1, outlier.shape = NA) + # do not show outlier in boxplot
  stat_summary(fun="mean", geom = "point", col = ambition_coral) + 
  stat_summary(fun.data = mean_se, geom = "errorbar", width = .05, col = ambition_coral) +
  ggdist::stat_dots(side = "left", dotsize = 0.3, justification = 1.1, binwidth = .1, col = ambition_charcole, fill = ambition_charcole) +
  coord_cartesian(ylim = c(-2,2), xlim = c(1,1)) +
  #scale_y_discrete(breaks = c("too easy", "", "neither nor", "", "too difficult")) +
  theme_bw() +
  theme(axis.title.x = element_blank(), axis.text.x = element_blank()) +
  ylab("Rating") +
  xlab("Items")
...
```{r, echo=FALSE, results='asis', fig.align='center', fig.width=10}
...
# create rain cloud plot, adapted from: https://z3tt.github.io/Rainclouds/
ggplot(desc, aes(x = variable, y = value)) +
  ggdist::stat_halfeye(adjust = 1.5, width = .3, .width = 0, justification = -.3, point_colour = NA, fill = ambition_charcole) + 
  geom_boxplot(width = .1, outlier.shape = NA) + # do not show outlier in boxplot
  geom_hline(yintercept = nrow(quest)/4, linetype="dashed") +
  stat_summary(fun="mean", geom = "point", col = ambition_coral) + 
