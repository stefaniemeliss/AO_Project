...
# q1
q01_q  <- "Reading comprehension can be described as relying on two capacities: the ability to recognise words and build meaning from words. These is generally described as..."
...
# q5
q05_q  <- "Alongside chunks of spoken sound, the English writing system represents the smallest chunks w05thin w5rds that are associated with meaning called..."
...
questions <- lapply(ls(pattern="_q"), get)
...
pretest <- do.call(rbind.data.frame, questions)
names(pretest) <- "questions"
...
knitr::kable(pretest, caption = "Pretest Questions with Correct Answers and Distractors")
...
q_rand <- pretest[sample(1:nrow(pretest)), ] # shuffle order of questions
names(q_rand) <- c("Question", "Option_1", "Option_2", "Option_3", "Option_4") # rename cols
...
```{r, results='asis', echo=F}
knitr::kable(q_rand, caption = "Pretest Questions Randomised")
